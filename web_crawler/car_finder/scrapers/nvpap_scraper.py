...
class NVPAPScraper(BaseScraper):
    """Scraper for NVPAP Honda Insight listings."""
    
    def __init__(self):
        super().__init__("NVPAP")
        self.base_url = "https://nvpap.com/part-interchange/"
        self.search_url = "https://nvpap.com/part-interchange/"
        
    def scrape_listings(self) -> List[Vehicle]:
        """Scrape Honda Insight listings from NVPAP."""
        logger.info("Starting NVPAP scraping for Honda Insight")
        ...
        try:
            ...
            logger.info(f"Found {len(vehicles)} Honda Insight listings on NVPAP")
            return vehicles
            
        except Exception as e:
            logger.error(f"Error parsing NVPAP page: {e}")
            return []
    
    def _search_for_insights(self, soup: BeautifulSoup) -> List[Vehicle]:
        """Submit a search for Honda Insight vehicles."""
        try:
            # Look for dropdown forms
            ...
            if search_form:
                ...
                # Build search URL
                if action:
                    search_url = action if action.startswith('http') else f"https://nvpap.com{action}"
                else:
            ...
            logger.error(f"Error submitting search to NVPAP: {e}")
            return []
    ...
    def _extract_vehicle_details(self, vin: str, html_content: str, soup: BeautifulSoup) -> Optional[Vehicle]:
        ...
        try:
            ...
            # Extract location - NVPAP locations
            location = self._extract_nvpap_location(context)
    ...
    def _extract_nvpap_location(self, context: str) -> Optional[str]:
        """Extract location information specific to NVPAP."""
        nvpap_patterns = [
            r'NVPAP\s*-?\s*([A-Za-z\s]+)',
            r'Location\s*:?\s*([A-Za-z\s,]+)',
            r'Address\s*:?\s*([A-Za-z\s,]+)'
        ]
        
        for pattern in nvpap_patterns:
